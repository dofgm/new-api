...
// GetAllBillingRecords 管理员获取全平台账单记录（支持状态筛选、关键词搜索、用户ID搜索和时间范围）
func GetAllBillingRecords(status string, keyword string, searchUserId int, startTime int64, endTime int64, pageInfo *common.PageInfo) (topups []*TopUp, total int64, err error) {
	tx := DB.Begin()
	if tx.Error != nil {
		return nil, 0, tx.Error
	}
...
// GetAllBillingStats 管理员获取全平台账单统计（只统计 success 状态，支持用户ID筛选）
func GetAllBillingStats(searchUserId int, startTime int64, endTime int64) (*BillingStats, error) {
	stats := &BillingStats{}

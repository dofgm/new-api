...
func RechargeXunhu(tradeNo string) (err error) {
	if tradeNo == "" {
		return errors.New("未提供支付单号")
	}
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		if topUp.PaymentMethod != "wxpay" && topUp.PaymentMethod != "alipay" {
			return ErrPaymentMethodMismatch
		}
	...
	if quotaToAdd > 0 {
		RecordTopupLog(topUp.UserId, fmt.Sprintf("虎皮椒充值成功，充值额度: %v，支付金额: %.2f", logger.FormatQuota(quotaToAdd), topUp.Money), "", topUp.PaymentMethod, "xunhu")
	}
